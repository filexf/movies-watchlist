...
import { loadMoreMovies, type Movie } from '../store/slices/moviesSlice'
...
const CategoryPage = ({
  ...
  const { currentPage, totalPages, loadingMore } = useSelector((state: RootState) => state.movies)
  ...
  return (
    <div className="mb-8">
      ...
      <div className="mt-8">
        {loadingCategory && <div className="text-center text-sky-400">Chargement...</div>}
        ...
        {!loadingCategory && categoryMovies.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {categoryMovies.map((movie, index) => (
              <div
                key={movie.id}
                ref={index === categoryMovies.length - 1 ? lastMovieElementRef : null}
        ...
        {loadingMore && (
          <div className="text-center text-sky-400 mt-4">Chargement de plus de films...</div>
        )}

        {!loadingCategory && categoryMovies.length === 0 && category && (
          <div className="text-center text-gray-400">Aucun film trouvé dans cette catégorie</div>
        )}
      </div>
    </div>
  )
